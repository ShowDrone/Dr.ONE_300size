...
// 반시계 방향 a-b-c-d
#define SERV0_A_PINNUM 6
#define SERV0_B_PINNUM 9
#define SERV0_C_PINNUM 10
#define SERV0_D_PINNUM 11
#define BLDC_TOP_PINNUM 3
#define BLDC_BOTTOM_PINNUM 5 //
...
Servo bl0;
Servo bl1;
SERVO x1  = { SERV0_A_PINNUM, 700, 0 };    // roll_A
SERVO x2  = { SERV0_C_PINNUM, 700, 0 };    // roll_B
SERVO y1 = { SERV0_B_PINNUM, 700, 0 };     // pitch_A
SERVO y2 = { SERV0_D_PINNUM, 700, 0 };     // pitch_B
...
void setup() {
  ...
  roll_A.attach(x1.pinNum);
  roll_B.attach(x2.pinNum);
  pitch_A.attach(y1.pinNum);
  pitch_B.attach(y2.pinNum);
...
void loop() {
  ...
  /*x1.control = x2.control = map(buf[1], 88, 168, 45, 135);           //  roll 조이스틱 데이터
    y1.control = y2.control = map(buf[2], 88, 168, 45, 135);           //  pitch 조이스틱 데이터
  ...
  */
  ...
  if (1000 <= input && input <= 2000)
    prop = input;
  else if (45 <= input && input <= 135)
    paddle = input;

  x1.control = map(paddle, 45, 135, 135, 45);
  x2.control = map(paddle, 45, 135, 30, 120);
  y1.control = map(paddle, 45, 135, 120, 30);
  y2.control = map(paddle, 45, 135, 30, 120);
  bl0.writeMicroseconds(prop);
  bl1.writeMicroseconds(prop);

  roll_A.write(x1.control);
  roll_B.write(x2.control);
  pitch_A.write(y1.control);
  pitch_B.write(y2.control);
  setPidBLDC(&yaw);
...
void debugDisplay() {
  ...
  */
  ...
  Serial.print(x1.control); Serial.print("\t");
  Serial.print(y1.control); Serial.print("\t");
